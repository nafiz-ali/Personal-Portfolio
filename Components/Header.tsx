        ...
        <div className="hidden md:flex items-center space-x-2">
          {NAV_LINKS.map(link => (
            <a 
              ...
              className={`px-3 py-2 rounded-md text-lg font-medium transition-all duration-300 hover:text-cyan-300 hover:bg-cyan-400/10 ${activeSection === link.id ? 'bg-cyan-400/20 text-cyan-300 text-glow' : 'text-gray-300'}`}
            >
              {link.title}
            </a>
          ))}
        ...
        <div className="flex flex-col items-center py-4 space-y-4">
          {NAV_LINKS.map(link => (
            <a 
              ...
              href={`#${link.id}`} 
              onClick={(e) => handleLinkClick(e, link.id)}
              className={`text-lg transition-colors duration-300 hover:text-cyan-400 ${activeSection === link.id ? 'text-cyan-400 text-glow' : 'text-gray-300'}`}
            >
              {link.title}
            </a>
          ))}
